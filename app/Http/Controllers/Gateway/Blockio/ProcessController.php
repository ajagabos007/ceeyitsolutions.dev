...
class ProcessController extends Controller
{
    ...
    public function ipn()
    {
        $DepositData = Deposit::where('status', 0)->where('method_code', 502)->where('try', '<=', 100)->where('btc_amo', '>', 0)->where('btc_wallet', '!=', '')->orderBy('id','desc')->get();

        foreach ($DepositData as $data) {

            $blockIoAcc = json_decode($data->gatewayCurrency()->gateway_parameter);
            $apiKey = $blockIoAcc->api_key;
            $version = 2;
            $pin = $blockIoAcc->api_pin;
            $block_io = new BlockIo($apiKey, $pin, $version);
